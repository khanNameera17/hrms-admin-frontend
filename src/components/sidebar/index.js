...
import { TbTargetArrow, TbHierarchy3, TbMatrix, TbUsersGroup } from "react-icons/tb";
import { RiTimeLine, RiPinDistanceFill } from "react-icons/ri";
import logo from "../../assets/images/company-logo.png";
import "./style.scss";
import { FcGlobe } from "react-icons/fc";
import { VscOrganization } from "react-icons/vsc";
import { GiOrganigram, GiReceiveMoney } from "react-icons/gi";
import { IoMdPersonAdd } from "react-icons/io";
import { HiOutlineFingerPrint } from "react-icons/hi";
import { PiUsers, PiUserSwitchLight } from "react-icons/pi";
...
function Sidebar({ isCollapsed, open, toggleSidebar }) {
  ...
  const navItems = useMemo(() => ([
    ...
    // HR
    {
      ...
      onClick: () => open(),
      children: [
        { name: "Attendance", to: "/attendance", icon: <HiOutlineFingerPrint /> },
        { name: "Attendance Matrix", to: "/attendance-matrix", icon: <TbMatrix /> },
        { name: "Leave Application", to: "/leaveApplication" },
        { name: "Payroll", to: "/payroll", icon: <GiReceiveMoney />},
          { name: "Firms", to: "/firm", icon: <GoOrganization /> },
      ],
    },
    // user
    {
      ...
      children: [
        // { name: "Super Admin", to: "/super-admin" },
        // { name: "Admin", to: "/admin" },
        { name: "Employee", to: "/employee", icon: <PiUserSwitchLight/> },
        ...(role === "super_admin" ? [{ name: "Admin", to: "/admin", icon: <PiUsers /> }] : []),
      ],
      onClick: () => open(),
    },
    // config - hierarchy, user, etc
    {
      name: "Configuration",
      to: "#",
      icon: (
      ...
      onClick: () => open(),
      children: [
       // ...(role === "super_admin" ? [{ name: "Organization", to: "/organization", icon: <PiUsers /> }] : []),
       { name: "Actor Type Hierarchy", to: "/actorTypeHierarchy", icon: <TbHierarchy3 /> },
        { name: "Hierarchy", to: "/hierarchy", icon: <GiOrganigram /> },
        { name: "Add User", to: "/addUser", icon: <IoMdPersonAdd /> },
        { name: "Meta Data", to: "/metadata", icon: <IoMdPersonAdd /> },
      ],
    },
